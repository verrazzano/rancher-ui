...
export default Component.extend(ClusterDriver, {
  ...
  actions: {
    saveStep1(cb) {
      ...
      return hash(ociRequest).then((resp) => {
        const { nodeShapes, nodeImages } = resp;

        const ol8Shapes = (get( nodeImages, 'body') || [] ).filter((image) => image.startsWith('Oracle-Linux-8') && !image.includes('aarch64'))

        setProperties(this, {
          nodeShapes:           (get( nodeShapes, 'body') || [] ).reverse(),
          // Only bring in Oracle Linux 8 Platform Images
          nodeImages:           ol8Shapes,
          errors:               [],
        });
        set(this, 'cluster.ociocneEngineConfig.imageDisplayName', ol8Shapes[0])
        set(this, 'step', 2);
        cb(true);
      }).catch((xhr) => {
  ...
  canCreateCluster: computed('config.nodeShape', 'config.imageDisplayName', function() {
    return !(get(this, 'config.nodeShape') && get(this, 'config.imageDisplayName'));
  }),
