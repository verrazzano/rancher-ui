...
export default Component.extend(ClusterDriver, {
  ...
  nodeImageOptions: computed('nodeImages', 'config.kubernetesVersion', function() {
    ...
    let version = kubernetesVersion.replace('v', '')

    nodeImages = nodeImages.filter((image) => image.includes(version))
  ...
  validate() {
    ...
    if (!get(this, 'config.imageDisplayName')) {
      errors.push('Node image is required')
    }
    if (get(this, 'config.imageDisplayName')) {
      let nodeImageName = get(this, 'config.imageDisplayName');
      let kubernetesVersion = get(this, 'config.kubernetesVersion');
      let version = kubernetesVersion.replace('v', '')

      if (!nodeImageName.includes(version)) {
        errors.push('Node image version must match Kubernetes version')
      }
    }
