...
export default Component.extend(ClusterDriver, {
  ...
  canSaveNetworking: computed('config.{podSubnet,loadBalancerSubnet,workerNodeSubnet,controlPlaneSubnet,vcnId}', 'vcnCreationMode', function() {
    const mode = get(this, 'vcnCreationMode')

    switch (mode) {
    case 'Existing':
      return !(get(this, 'config.podSubnet') && get(this, 'config.workerNodeSubnet') && get(this, 'config.controlPlaneSubnet') && get(this, 'config.loadBalancerSubnet') && get(this, 'config.vcnId'))
    case 'Quick':
      return false
    default:
      return true;
  ...
  validate() {
    ...
    if (get(this, 'vcnCreationMode') === 'Existing') {
      ...
      if (!get(this, 'config.podSubnet')) {
        errors.push('Pod subnet is required')
      }
    }
